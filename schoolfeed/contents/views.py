...
class Contents(APIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            try:
                schools_models.Member.objects.get(school__id=request.data.get('school'), member=user, role__gte=0)
            except (schools_models.Member.DoesNotExist):
...
class ContentsDetail(APIView):
    ...
    def find_managing_school(self, school_id, user):
        try:
            ...
            return school
        except (schools_models.School.DoesNotExist, schools_models.Member.DoesNotExist):
            return None
